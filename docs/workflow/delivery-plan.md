...
6. Release 워크플로 자동 실행 → release용 deb 빌드 + amd64/arm64 smoke-test + tagged `amd64`/`arm64` `.deb` 설치형 reusable integration workflow
...
```bash
# 1. 인터넷 되는 머신에서 최신 deb 다운로드 (아키텍처 자동 감지)
...
sha256sum -c "rclone-azureblob-airgap_${VER}_${ARCH}.deb.sha256"  # 선택: 무결성 검증
